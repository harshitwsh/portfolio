...
ws.onopen = async () => {
    ...
    expression: `
      (() => {
        ...
        hm.group.traverse(obj => {
          if (obj.isBone && (obj.name.includes('hand') || obj.name.includes('Hand') || obj.name.includes('wrist') || obj.name.includes('Wrist') || obj.name.includes('root'))) {
            let wp = new obj.position.constructor();
  ...
  console.log('HANDS BONE POSITIONS:', JSON.stringify(info.result?.value, null, 2));
