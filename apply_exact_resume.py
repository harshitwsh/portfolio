...
def generate_exact_resume_modal():
    modal_css = """
    .resume-backdrop {
    ...
    .rm-about-text {
      margin: 12px 0 0 0; font-size: 12.8px; line-height: 1.55; color: #9ca3af;
    }
    .rm-exp-title {
      font-size: 13.5px; font-weight: 700; color: #ffffff; margin-top: 12px;
    }
    .rm-exp-meta {
      font-size: 12px; font-weight: 600; color: #4ade80; margin-top: 2px;
    }
    .rm-exp-bullets {
      margin: 8px 0 0 0; padding-left: 14px;
    ...
    .rm-edu-title {
      font-size: 13.5px; font-weight: 700; color: #ffffff; margin-top: 12px; line-height: 1.35;
    }
    .rm-edu-univ {
    ...
    .rm-pcard-tech {
      font-size: 10.5px; color: #6b7280; margin-top: 10px;
    ...
    .rm-ibuild-header {
      color: #4ade80; font-size: 12.5px; font-weight: 700; letter-spacing: 0.06em;
...
function injectResumeModalStyles() {{
  if (document.querySelector('style[data-resume-modal="1"]')) return;
...
let _activeResumeModal = null;
function openResumeModal() {{
  if (!_activeResumeModal) {{
    _activeResumeModal = createResumeModal();
  }}
  _activeResumeModal._backdrop.classList.add("is-active");
  _activeResumeModal.classList.add("is-active");
}}

function closeResumeModal() {{
  if (_activeResumeModal) {{
    _activeResumeModal._backdrop.classList.remove("is-active");
    _activeResumeModal.classList.remove("is-active");
  }}
}}
"""
    return js_code_block
...
for filepath in target_files:
    ...
    # Find where injectResumeModalStyles starts
    start_pos = code.find("function injectResumeModalStyles()")
    end_pos = code.find("function closeResumeModal()", start_pos)
    if start_pos != -1 and end_pos != -1:
        # Find closing brace of closeResumeModal
        end_brace = code.find("}", end_pos)
        end_pos_full = code.find("}", end_brace + 1) + 1
        
        replacement = generate_exact_resume_modal()
        new_code = code[:start_pos] + replacement + code[end_pos_full:]
        with open(filepath, "w", encoding="utf-8") as f:
            f.write(new_code)
        print("Successfully updated resume modal in:", filepath)
    else:
        print("Could not find start/end position in:", filepath)
